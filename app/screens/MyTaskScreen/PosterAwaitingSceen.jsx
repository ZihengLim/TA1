...
const NoPostsIcon = () => (
  <View style={styles.noPostsContainer}>
    ...
    <Text style={styles.noPostsText}>No Offer Available...</Text>
  </View>
);
  ...
  return (
    <View style={styles.lower}>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('ReviewOffer')}>
...
const PosterAwaitingScreen = () => {
  ...
  const renderedTaskIDsRef = useRef(new Set());
  ...
  useEffect(() => {
    ...
    const unsubscribe = firestore()
      ...
      .onSnapshot(querySnapshot => {
        const newTasks = querySnapshot.docs
          .filter(doc => !renderedTaskIDsRef.current.has(doc.id))
          .map(doc => ({
            ...doc.data(),
            id: doc.id,
          }));

        newTasks.forEach(task => renderedTaskIDsRef.current.add(task.id));
